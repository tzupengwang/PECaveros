#include <bits/stdc++.h>
using namespace std;
const int N = 300010;
struct SA{
#define REP(i,n) for ( int i=0; i<int(n); i++ )
#define REP1(i,a,b) for ( int i=(a); i<=int(b); i++ )
  bool _t[N*2];
  int _s[N*2], _sa[N*2], _c[N*2], x[N], _p[N], _q[N*2], hei[N], r[N];
  int operator [] (int i){ return _sa[i]; }
  void build(int *s, int n, int m){
    memcpy(_s, s, sizeof(int) * n);
    sais(_s, _sa, _p, _q, _t, _c, n, m);
    mkhei(n);
  }
  void mkhei(int n){
    REP(i,n) r[_sa[i]] = i;
    hei[0] = 0;
    REP(i,n) if(r[i]) {
      int ans = i>0 ? max(hei[r[i-1]] - 1, 0) : 0;
      while(_s[i+ans] == _s[_sa[r[i]-1]+ans]) ans++;
      hei[r[i]] = ans;
    }
  }
  void sais(int *s, int *sa, int *p, int *q, bool *t, int *c, int n, int z){
    bool uniq = t[n-1] = true, neq;
    int nn = 0, nmxz = -1, *nsa = sa + n, *ns = s + n, lst = -1;
#define MS0(x,n) memset((x),0,n*sizeof(*(x)))
#define MAGIC(XD) MS0(sa, n); \
    memcpy(x, c, sizeof(int) * z); \
    XD; \
    memcpy(x + 1, c, sizeof(int) * (z - 1)); \
    REP(i,n) if(sa[i] && !t[sa[i]-1]) sa[x[s[sa[i]-1]]++] = sa[i]-1; \
    memcpy(x, c, sizeof(int) * z); \
    for(int i = n - 1; i >= 0; i--) if(sa[i] && t[sa[i]-1]) sa[--x[s[sa[i]-1]]] = sa[i]-1;
    MS0(c, z);
    REP(i,n) uniq &= ++c[s[i]] < 2;
    REP(i,z-1) c[i+1] += c[i];
    if (uniq) { REP(i,n) sa[--c[s[i]]] = i; return; }
    for(int i = n - 2; i >= 0; i--) t[i] = (s[i]==s[i+1] ? t[i+1] : s[i]<s[i+1]);
    MAGIC(REP1(i,1,n-1) if(t[i] && !t[i-1]) sa[--x[s[i]]]=p[q[i]=nn++]=i);
    REP(i, n) if (sa[i] && t[sa[i]] && !t[sa[i]-1]) {
      neq=lst<0||memcmp(s+sa[i],s+lst,(p[q[sa[i]]+1]-sa[i])*sizeof(int));
      ns[q[lst=sa[i]]]=nmxz+=neq;
    }
    sais(ns, nsa, p + nn, q + n, t + n, c + z, nn, nmxz + 1);
    MAGIC(for(int i = nn - 1; i >= 0; i--) sa[--x[s[p[nsa[i]]]]] = p[nsa[i]]);
  }
}sa;
int H[ N ], SA[ N ];
void suffix_array(int* ip, int len) {
  // should padding a zero in the back
  // ip is int array, len is array length
  // ip[0..n-1] != 0, and ip[len] = 0
  ip[len++] = 0;
  sa.build(ip, len, 128);
  for (int i=0; i<len; i++) {
    H[i] = sa.hei[i + 1];
    SA[i] = sa._sa[i + 1];
  }
  // resulting height, sa array \in [0,len)
}
#define N 101010
#define L(X) (X<<1)
#define R(X) (1+(X<<1))
#define mid ((l+r)>>1)
int st[ N << 2 ];
void reset( int no , int l , int r ){
  st[ no ] = 0;
  if( l == r ) return;
  reset( L( no ) , l , mid );
  reset( R( no ) , mid + 1 , r );
}
void modify( int no , int l , int r , int p , int d ){
  if( l == r ){
    st[ no ] = d;
    return;
  }
  if( p <= mid )
    modify( L( no ) , l , mid , p , d );
  else
    modify( R( no ) , mid + 1 , r , p , d );
  st[ no ] = st[ L( no ) ] + st[ R( no ) ];
}
int query( int no , int l , int r , int kth ){
  if( kth > st[ no ] ) return -1;
  if( l == r ) return l;
  if( kth <= st[ L( no ) ] ) return query( L( no ) , l , mid , kth );
  return query( R( no ) , mid + 1 , r , kth - st[ L( no ) ] );
}
char s[ N ];
int ss[ N ] , n;
struct DJS{
  int p[ N ] , l[ N ] , r[ N ] , v[ N ];
  void reset(){
    for( int i = 0 ; i <= n ; i ++ ){
      p[ i ] = i;
      l[ i ] = i;
      r[ i ] = i;
      v[ i ] = SA[ i ];
    }
  }
  int f( int x ){
    return x == p[ x ] ? p[ x ] : p[ x ] = f( p[ x ] );
  }
  void uni( int x , int y ){
    x = f( x );
    y = f( y );
    if( x == y ) return;
    p[ x ] = y;
    l[ y ] = min( l[ y ] , l[ x ] );
    r[ y ] = max( r[ y ] , r[ x ] );
    v[ y ] = min( v[ y ] , v[ x ] );
  }
} djs;
int q , at[ N ];
vector<int> wt[ N ];
vector< pair<int,int> > qq[ N ];
void init(){
  scanf( "%s" , s );
  n = strlen( s );
  for( int i = 0 ; i <= n ; i ++ ){
    wt[ i ].clear();
    qq[ i ].clear();
  }
  scanf( "%d" , &q );
  for( int i = 0 ; i < q ; i ++ ){
    int qk , qm;
    scanf( "%d%d" , &qk , &qm );
    qq[ qk ].push_back( { qm , i } );
  }
  for( int i = 0 ; i < n ; i ++ )
    ss[ i ] = s[ i ] - 'a' + 1;
  suffix_array( ss , n );

  for( int i = 0 ; i < n ; i ++ )
    at[ SA[ i ] ] = i;
  for( int i = 1 ; i < n ; i ++ )
    if( H[ i ] )
      wt[ H[ i ] ].push_back( i );
}
int ans[ N ];
void solve(){
  reset( 1 , 0 , n - 1 );
  djs.reset();
  for( int i = n ; i >= 1 ; i -- ){
    int who = at[ n - i ];
    modify( 1 , 0 , n - 1 , who , 1 );
    for( int j : wt[ i ] ){
      modify( 1 , 0 , n - 1 , j , 0 );
      djs.uni( j - 1 , j );
    }
    for( auto qqq : qq[ i ] ){
      int qm = qqq.first;
      int qid = qqq.second;
      if( st[ 1 ] < qm )
        ans[ qid ] = -1;
      else{
        int aa = query( 1 , 0 , n - 1 , qm );
        ans[ qid ] = djs.v[ djs.f( aa ) ];
      }
    }
  }
  for( int i = 0 ; i < q ; i ++ )
    if( ans[ i ] == -1 )
      puts( "Not found" );
    else
      printf( "%d\n" , ans[ i ] ); 
}
int main(){
  int t; scanf( "%d" , &t ); while( t -- ){
    init();
    solve();
  }
}
