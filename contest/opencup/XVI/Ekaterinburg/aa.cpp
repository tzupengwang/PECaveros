...
// tools
  ...
  void add_point(const Pt& p){ 
    TriRef tt = find(the_root, p);
    //assert( tt->contains( p ) and !tt->has_chd() );
    //if( n > 300 ) return;
    add_point(tt, p);
  }
// tools
