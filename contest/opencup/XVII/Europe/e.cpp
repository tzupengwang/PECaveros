...
typedef tuple<bigN,bigN,bigN> XD;
tuple<bigN,bigN,bigN> fix(bigN a, bigN b, bigN c){
  if( b < a ) swap( a , b );
  if( c < a ) swap( a , c );
  if( c > b ) swap( b , c );
...
inline void expand( bigN& A , bigN& B , bigN& C ) {
  bigN hao = SQRT( K*K*(B+C)*(B+C)-(B*B+C*C+K*B*C+1)*4 );
  bigN Ap = (K*(B+C)+hao)/2;
  bigN An = (K*(B+C)-hao)/2;
  if( !st1.count(fix(Ap,B,C)) ) {
    st1.insert(fix(Ap,B,C));
    qu.push(XD(Ap,B,C));
  }
  if( !st1.count(fix(An,B,C)) ) {
    st1.insert(fix(An,B,C));
    qu.push(XD(An,B,C));
  }
}
...
int main(){
  cin >> k >> n;
  K = bigN(k);
  qu.push(XD{bigN(1),K,K*(K+1)});
  st1.insert(XD{bigN(1),K,K*(K+1)});
  while(qu.size()) {
    XD cur = qu.front();qu.pop();
    bigN A , B , C;
    tie( A , B , C ) = cur;
    if ( A > 0 and B > 0 and C > 0 ) {
      if( !st2.count( A ) and !st2.count( B ) and !st2.count( C )  ) {
        ans.push_back( cur );
        st2.insert( A );
        st2.insert( B );
        st2.insert( C );
